...
def visualize_camera_and_ground_truth(estimated, aligned_gt,
                                      ...
                                      scale=0.2):
    """
    estimated    : dict mapping timestamp -> estimated state (np.array of length 9)
    aligned_gt   : dict mapping timestamp -> ground-truth [x,y,z,roll,pitch,yaw]
    frames       : list/array of frame dicts (with keys 'id','p1','p2','p3','p4','t')
    K            : camera intrinsics (3×3)
    dist         : distortion coefficients (length 5)
    tag_matrix   : Apriltag layout matrix
    scale        : axis length for drawing orientation axes
    """
    ...
    class EKF:
        def __init__(self, Q, R, x0=None, P0=None):
            self.Q = Q; self.R = R
            self.x = np.zeros((9,1)) if x0 is None else x0.reshape(9,1)
            self.P = np.eye(9)*1e-2 if P0 is None else P0
    ...
    class ParticleFilter:
        def __init__(self, num_particles, Q, R, init_mean, init_cov):
            ...
            self.Q  = Q
            self.R  = R
            self.particles = np.random.multivariate_normal(init_mean.flatten(),
                                                        init_cov,
                                                        size=self.np)
        ...
        def predict(self, dt):
            ...
            self.particles = (self.particles @ F.T
                            + np.random.multivariate_normal(np.zeros(9),
                                                            self.Q,
                                                            size=self.np))

        def update(self, z):
            hx = np.hstack((self.particles[:,0:3],
                            self.particles[:,6:9]))  # (np,6)
            y = hx - z.reshape(1,6)
        ...
        def estimate(self):
            ...
            x_wmean = np.average(self.particles,
                                axis=0,
...
if __name__ == "__main__":
    ...
    # load all data
    K, dist_coeffs, tag_matrix = read_parameters("Module_2_PnP/data/parameters.txt")
    mat      = sio.loadmat("Module_2_PnP/data/studentdata" + traj + ".mat", simplify_cells=True)
    frames   = mat['data']
    ...
    for frm in frames:
        ...
        init_z    = np.hstack((dp, [roll, pitch, yaw], np.zeros(3)))
        init_time = frm['t']
        break
    ...
    if filt == 'ekf':
        ekf = EKF(Q=Q_PROC, R=R_MEAS, x0=init_z, P0=np.eye(9)*1e-2)
        ...
        pf        = ParticleFilter(num_p, Q_PROC, R_MEAS, init_z, init_cov)
        estimated[init_time]    = init_z
